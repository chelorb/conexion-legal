...
// Lee de documentos_abogado con estado real y permite aprobar/rechazar
// ─────────────────────────────────────────────────────────────
function TabDocumentos({ usuario }) {
  ...
  const cargar = async () => {
    setCargando(true);
    try {
      const { data } = await api.get(`/documentos/abogado/${usuario.id}`);
  ...
  const revisar = async (docId, estado) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Error al procesar el documento.');
      ...
      {docs.map(doc => {
        ...
        return (
          <div key={doc.id} className="rounded-xl p-3 space-y-2"
            ...
            {/* Fila superior: nombre + estado + ver */}
            ...
            {doc.estado === 'pendiente' && (
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => revisar(doc.id, 'aprobado')}
                  disabled={enProceso}
                ...
                <button
                  onClick={() => revisar(doc.id, 'rechazado')}
                  disabled={enProceso}
                  className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg font-body text-xs font-medium transition-colors disabled:opacity-40"
